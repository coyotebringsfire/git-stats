var Q=require('q'),
	debug=require('debug')('git:clone'),
	async=require('async'),
	merge=require('merge'),
	child_process=require('child_process');

module.exports=function doClone(repo, _options) {
	var debug=require('debug')('git:clone:doClone'),
		defaultOptions = {
			targetDirectory:"/tmp/"
	...
	if(!repo) {
		...
		setTimeout(function onTimeout() {
			var debug=require('debug')('git:clone:doClone:onTimeout');
	...
	child_process.exec('git clone ' + repo, function onGitCloneExit(err, stderr, stdout) {
		var debug=require('debug')('git:clone:doClone:onGitCloneExit');
	    if (err) {
