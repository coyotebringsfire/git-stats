...
function processCommits(commits) {
	...
	var processPromise=Q.defer(), commits.commitAuthors=[];
	...
	async.eachLimit(10, Object.keys(commits),function forEachCommit(commit,done) {
		var _commit=commit;
		var debug=require('debug')('git:processCommits:forEachCommit');
		...
		gitShow.show(sha)
			.then( function onGitShowSuccess(output) {
				var debug=require('debug')('git:processCommits:forEachCommit:onGitShowSuccess');
				commits[commit].showStats=output;
				done();
			}, function onGitShowError(err) {
				var debug=require('debug')('git:processCommits:forEachCommit:onGitShowError');
				done(err);
			});
