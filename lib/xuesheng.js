...
process.on('message', function onChildMessageReceived(msg) {
  var debug=require('debug')('xuexi:onChildMessageReceived');
  debug("msg= %j", msg);
  async.eachLimit(msg, 1, function forEachModule(module, nextModule) {
    ...
    nr.npmStats.get(module)
      .then(function onGetResolved(details) {
        var debug=require('debug')('xuexi:onChildMessageReceived:onGetResolved');
        var module_details={
        ...
        if( module_details.repository && module_details.repository.type === "git" ) {
          ...
          clone(module_details.repository.url, {targetDirectory:"/tmp/xuexi"})
            .then(function onCloneResolved(results) {
              var debug=require('debug')('xuexi:onChildMessageReceived:onGetResolved:onCloneResolved'), repoPath=util.format("/tmp/xuexi/%s", module);
              ...
              rimraf(repoPath, function(err) {
                ...
                process.send({module:module, details:module_details});
                nextModule();
              });
              */
        ...
        var debug=require('debug')('xuexi:onChildMessageReceived:onGetRejected');
