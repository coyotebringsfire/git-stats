...
module.exports=function processCommits(commits) {
	...
	async.eachLimit(Object.keys(commits), 1, function forEachCommit(commit,done) {
		...
		var KEYS={
			...
			"AVERAGELINESDELETED":1,
			"AVERAGELINESADDED":2,
			"STDDEVIATIONLINESCHANGED":3,
			"STDDEVIATIONLINESDELETED":4,
			"STDDEVIATIONLINESADDED":5, 
			"VARIANCELINESCHANGED":6,
			"VARIANCELINESDELETED":7,
			"VARIANCELINESADDED":8, 
			"NUMAUTHORS":9
		}, response=[];
		response[KEYS["AVERAGELINESCHANGED"]]=1/commits.averageLinesChanged || 0;
		response[KEYS["AVERAGELINESADDED"]]=1/commits.averageLinesAdded || 0;
		response[KEYS["AVERAGELINESDELETED"]]=1/commits.averageLinesDeleted || 0;

		response[KEYS["STDDEVIATIONLINESCHANGED"]]=1/commits.standardDeviationLinesChanged || 0;
		response[KEYS["STDDEVIATIONLINESADDED"]]=1/commits.standardDeviationLinesAdded || 0;
		response[KEYS["STDDEVIATIONLINESDELETED"]]=1/commits.standardDeviationLinesDeleted || 0;

		response[KEYS["VARIANCELINESCHANGED"]]=1/commits.varianceLinesChanged || 0;
		response[KEYS["VARIANCELINESADDED"]]=1/commits.varianceLinesAdded || 0;
		response[KEYS["VARIANCELINESDELETED"]]=1/commits.varianceLinesDeleted || 0;

		response[KEYS["NUMAUTHORS"]]=commits.commitAuthors.length;

		//debug("resolving promise %j", commits);
		//_this.processedCommits=commits;
		debug("resolving promise %j", response);
		processPromise.resolve(response);
	});
